...
- **One person, one account, however many doors.** `user_identities` (`user_id`, `provider`, `provider_id`, unique on the pair) holds the ways in; `users` holds who somebody is (#269). Sign-in keys on the identity, never on `users.provider`, which now means only how an account was **born** — `'local'` is what makes it a password account, and that is all it still gates (2FA, password recovery). Migration 20 scoped the email index to `provider = 'local'` deliberately, saying *"we don't link accounts yet"*; that deferred decision came due when signing in with Google after registering with a password made a **second** account on the same address. `resolveIdentity` (`auth/oauth.go`) is the one path every door goes through, dev login included, and reads in order: **the door we know** → refresh and enter; **a new door onto an address somebody has *proven*** → hang it there, same person; **nobody holds the address** → a new person. The refusal is the fourth case — the address is held but never confirmed — because an unverified account may be a squatter, and linking would hand it over with everything in it. An address now names exactly one account (`idx_users_email` over `lower(email)`), so registration refuses a taken one **and names the door to use instead**: "already exists" left people stranded at the wrong one of two buttons, and this app is a private tavern rather than a public directory, so disclosing which provider is worth it. A provider's address is never written over an account that already has one — a provider changing its mind must not move an address off somebody else — but an account with **no** address adopts the first one vouched for. Migration 57 resolved the pairs that already existed by the same rule read backwards (**first holder keeps it**), freeing the rest without deleting or moving anything and writing each one into `admin_actions` as `email_freed`, since a freed account silently loses password recovery — the procedure is in `docs/RUNBOOK.md`.
- **A friendship belongs to the account, not to a table.** Campaign chat already existed and is called the Chronicle (`AddChronicleNote` writes a player's line as `player_note`), so #181 built only what it never covered: knowing somebody *outside* a campaign, and saying something to one person or one party. `friendships` holds one row per pair **in the direction it was asked** — who asked is what a pending request *is* — with a unique index over `(least, greatest)` so two people asking each other cannot become two rows; when they do ask each other, the second ask **accepts** rather than duplicating. `user_blocks` is directional, is not a friendship state, and survives the friendship being dropped. Discovery is a **code you hand out, never a search**: `users.friend_code` mirrors the campaign invite code, because a search box over accounts is an enumeration door onto a private tavern and an account born through Google has no username to be found by. Every refusal is the same **404** — a code nobody holds, a code held by somebody who blocked you, and a room you do not ride with all answer alike, since a block that can be detected by probing is not one. The alphabet drops O/0/I/1/L in **all three** places a code is drawn (the backfill, the column default, and Go's `newFriendCode`), because a code is read aloud. A conversation opens between friends **or table-mates** — the social graph here is mostly the campaigns you sit at — and reading a thread that already exists survives the friendship ending, because a friendship is not the conversation's to erase. `TopicFriends` and `TopicMessages` publish into a **person's** room (`user:<id>`, `ServeMeStream`) rather than a campaign's, since a nudge about a friendship belongs to an account and outlives every table it sits at; a party room's nudge rides its campaign's stream, and the topic carries nothing, so the members who may not read it learn nothing from hearing it.
- **A table counts in its own coin.** `campaigns.coinage` is a JSONB ladder of `{name, abbrev, value}` (#195); NULL means the standard cp/sp/ep/gp/pp, which is what every campaign had when #174 shipped gold-only. It is JSONB and not a table because a ladder is read whole on every price, never queried across, and edited whole. **Everything is reckoned in BASE units — the ladder's smallest coin** — because that is the only unit every price and every purse can be stated in exactly; `priceBase` and `formatCoins` live in `backend/internal/http/money.go` and `frontend/src/lib/money.ts` and are held together by `fixtures/rules/price-coins.json` (which replaced `price-gold.json`). A sub-base remainder still rounds **up** — #174's handling fee. The purse is **`character_items.is_purse`**, a fact rather than the guess it used to be ("the first content-less row named exactly Gold Pieces", a rule spelled out in four places that a rename would have broken in all four at once) — and writing in a row named for the table's coin still makes one, when the hero has none. Two rules keep the reading human: the purse is named for the coin a table **talks in** (`goldRung` — gold where there is gold, the largest rung otherwise, because nobody calls their purse a purse of platinum), and a breakdown **counts down from that rung and never above it**, so 4,000 copper reads "40 gp" and never the arithmetically-identical "4 pp". Migration 59 multiplied every existing purse by 100 (gold → copper) since the worth is unchanged: 120 gp buys exactly what 12,000 cp buys. Changing a ladder **converts nothing** — an invented coin has no rate against the one it replaces — so the numbers stand and their meaning changes, which is why the screen says so before it lets a DM through. The printed 2024 sheet's five coin cells are filled **by rank** with the printed CP/SP/EP/GP/PP labels left alone: a DM running shards and crowns knows which column is which, and painting over Wizards' artwork to correct it would read as damage.
- **A road and a region are one thing wearing different clothes.** `map_shapes` (#262) holds an ordered run of normalised points and a `kind` — `line` is stroked along them, `area` is filled between them — so the street brush and the kingdom overlay are one table, one endpoint and one drawing gesture. Points are fractions of the image like a pin's x/y, and so are `width` and the dash, because a stroke measured in pixels is a hair on one map and a river on another. They are veiled **exactly as pins are**, which is the part worth keeping: DM-only means absent from the payload, and under fog a **line is clipped to its revealed runs** — a road appears mile by mile as the party walks it, and comes back as several shapes sharing one id (the client draws them, it never edits them). An **area is all-or-nothing** on any corner being seen, because dropping points from a polygon does not clip it, it redraws it into a different country; `dm_only` is what holds back a border the party should not yet have the shape of. A shape may name a `location`, so tinting Barovia makes it *open* Barovia. A `line` cannot be restyled into an `area` — rub it out and draw the other. Pins gained `shape` in the same issue (`pin` | circle | square | diamond | triangle | star | cross | skull), drawn as flat SVG geometry rather than the stroked interface icons because they are map furniture and must read at a quarter of an inch; only the teardrop hangs by its tip, everything else is centred on its spot. The fog brush floor also came down from 0.015 to 0.005 — it read as 3 on the dial and could not lift a single room.
- **A campaign stands on a realm, and the realm outlives it.** `realms` + `campaigns.realm_id` (**NOT NULL**, `ON DELETE RESTRICT`) is the container a setting is authored in, so one world can host a campaign measured in years and then the next one after it (#233). It is called a *realm* and not a world because "the world" already means the place tree inside a campaign, and two Worlds in one UI is not a vocabulary. Stage one is the container **only**: nothing moves up, and two campaigns in one realm see nothing whatever of each other — what it buys is the foreign key, so #234 (the atlas moves up) is a data migration rather than a schema invention. Backfill gave every campaign that already existed a realm of its own, taking the campaign's **own id** (names repeat; the id is the one thing already unique) and its name — which is why nothing reads differently until a second campaign stands on the same ground. That "until" is the display rule everywhere: the ledger bands campaigns under a realm only once it holds more than one, and the campaign header names the realm only when its name has diverged from the campaign's, exactly as the roster only bands heroes once there is more than one party. Ownership is **not** membership: a realm is listed, renamed, moved into and struck by whoever made it, and a co-DM runs the campaign without owning the ground under it — what reaches the rest of the table is the `realmName` riding on the campaign, because a table knowing what its setting is called is not a secret. `realms.named` records whether the owner has ever named one, and decides one thing: an emptied realm nobody named is swept away (on a move and on a disband alike, `sweepUnnamedRealm`) because it is only the dead campaign's name cluttering the founding picker, while a **named** one stands however empty — that is the ground the next campaign begins on, which is the entire point.
...
- **A monster says what it is, unless the DM says otherwise.** `encounter_combatants.player_label` is what the party reads; blank falls back to **"Unknown"** (`combatantForPlayer`). It decided the player's view from the day the tracker shipped and *nothing in the app ever set it* — no screen, no form, no default — so it stayed blank and every enemy read "Unknown" for good: a veil that was on with no way to lift it (#286). Two fixes, and the first is the one that matters: `snapshotFor` now defaults `PlayerLabel = Label`, so a combatant arrives named and the DM's single act of revealing it (the eye) is enough. Most fights are "three goblins attack", so the **real name is the default and the mystery is the thing you ask for** — the reverse of what shipped. The second is a control: `RevealName` in `rowParts.tsx` **states the answer rather than symbolising it** (`party sees "Goblin 1"` / `party sees Unknown`), because the eye beside it already means something else and a DM should not have to tell two eyes apart. The eye is **presence** — in the order at all; this is **identity** — and they are genuinely different questions, which is why a hidden combatant still shows its chip: that is the name they will read when it arrives. A mob shares one identity, so naming any names all, re-numbered per member the way the picker numbered them, and the chip quotes the name **without** the number because the party reads one line saying "Aboleth ×3". The pencil is `player_label`'s original purpose, still intact: a dragon the party may see but not yet name is a "Looming Shape". Existing encounters keep whatever they have — no migration — so "Name them all" sits in the tracker header, since ten monsters is ten clicks and that is the same complaint reworded.
- **A map that exists is a spoiler.** `maps.visible_to_party` + `map_visibility` per-hero overrides (#276) put the same veil on a map that a place, a notice, one of the Folk and a handout already carry — resolved through the viewer's own heroes, so the DM's control is literally `VisibilityControl` unchanged. Until it, fog covered the ground and left the map itself announcing that there *was* ground to cover: every member received every map, so the atlas listed the name of the dungeon the party had not found and `ServeMapImage` opened the picture for anyone holding the id. Two gates, no more: the map's own two layers, then the **place** it depicts (#229) walked to the root — veiling Barovia veils the city map of Barovia, and the walk is `veil.locationVisibleTo`, borrowed rather than reinvented. The map **tree** is deliberately *not* a third: `parent_map_id` says where a picture hangs, not what is known, and a DM who veils an overworld and forgets its dungeons should be told so by an atlas that still lists them rather than have them vanish by a rule nobody typed (the atlas already re-roots a map whose parent it cannot see). Redaction is absence in all three places a map is reachable — off the shelf, **404** from `GET /maps/{id}`, **404** from the image — and a link-pin leading into a veiled map goes with it, or the marker carries its name out. `toAPIMap` takes `forDM bool` for the invite code's reason (#207): every call site declares who is reading. Migration 61 adds the column `DEFAULT false` and *then* backfills every existing map to true, in that order and deliberately — nothing a table can see today disappears at deploy, while the **next** map hung is the DM's until they say otherwise, because a lair map is uploaded before the session it is found in.
- **You grab a shape by its edge.** #262's roads and regions shipped untouchable (#277): `ShapeForm`'s *Rub it out* was wired and correct, and no human could open the form, because `useMapViewer` captures the pointer for panning unless the press began on `button, [data-pin-id]` — and a captured pointer retargets every later event, so the browser resolved the click onto the container. Shapes join that exemption via `data-shape-id`, but **only along their stroke**: the fill is `pointer-events: none` and one generous invisible stroke along the run answers, plus the name. A region covering half the chart must still be ground you can drag, and a tap inside it still drops a pin or stamps fog. The other half is the **Inkwork** — what is drawn on this map, as a list, the way the Atlas lists what it is drawn on — because a hairline is a poor handle even when it answers: a road clipped away under fog, a region behind another, a run drawn off the screen have none to hunt. Pressing **Draw** opens it rather than dropping into line mode, which asks road-or-region one step earlier and costs no second toolbar button. The lesson worth keeping is why it escaped: the #262 specs were **all API-level**, so `map-shapes.spec.ts` now drives a real browser for the press.
- **Fog can hang on a place.** A reveal batch may name a location (#191), and then two gates stand in front of its ground: the heroes the batch was stamped for, and that place's veil resolved through the viewer's own heroes — ancestors included, exactly as the quest board resolves it. That is what makes "she grew up in Lisboa" expressible without a second veil system: the DM stamps the city once, hands the *place* to one hero, and the same stamps serve the whole table later when the place goes party-visible. The place gate lives in `filterRevealCircles` (`backend/internal/http/fog.go`), read by both the JSON map and the composited image through `playerRevealCircles`, so the two can never disagree about what a viewer may see. A batch with no place is the old behaviour, untouched. Deleting a place **cascades** to its batches rather than nulling them: a batch that lost its place would fall back to its audience alone, which would hand a struck city's ground to everyone it was stamped for. (`knowledge_pools` is gone — #232 re-keyed that first gate from users to heroes; the shape of the second one is unchanged.)
...
- **A sheet makes a person statted, never seated.** `characters.kind` (`hero` | `npc`) is the discriminator (#227), and `ListCharactersByCampaign` filters `kind = 'hero'` — which is why one line keeps the Folk's bodies out of the roster, the adventurer count, the Hall block, the chronicle, encounter seating and `seatedCharacterIDs` (the veil resolver) at once. A body is forged on the Folk page through `POST /npcs/{id}/body`: DM-owned, campaign-scoped, off `ListCharactersByOwner` and `CountSeatedByOwner`, and refused a seat. Because nothing else ever points at it, a body **dies with its person** — detaching it, putting a Den monster there instead, deleting the NPC or striking the campaign all destroy it (`strikeNpcBody`), or it would be a row reachable from nowhere. `table_born` therefore means one thing again: a stub for a guest with no account. A body is also **read** through its person and never through `campaigns.hidden_sheets` (#267) — that veil is about players not reading one another's heroes, and a body has no player, so asking it was wrong in both directions at once: with the veil down (nearly every table) it consulted nothing and handed the sheet to any member holding the id, and with it drawn it refused a sheet the DM had deliberately opened. `GET /characters/{id}` on a `kind = 'npc'` character now runs the Folk's own resolution instead (`bodyReadableBy`) — the DM, the ally's runner, else the stats veil through the viewer's heroes — and refuses with **404**, because a body you may not read must not be tellable from one that never was. The invariant that makes "through its person" singular is a partial unique index on `npcs.character_id`. And #250's "only a forged sheet is worth opening" retired with this: it was written when an NPC's sheet was a seated hero, and a body can never carry a `class_id`, so the test had quietly come to mean *never*.
- **A party is a brush, not a gate.** `parties` + `characters.party_id` (#232) are named groups of heroes for a table that splits across session groups. Every veil control has three grains — `scope: table | party | character` (the wire value `party` used to mean *everyone*; it is `table` now) — but only two of them reach the database: choosing a party writes the very same per-hero rows the DM could click one at a time, for whoever rides with it *at that moment*, and is then forgotten. Nothing downstream has ever heard of a party, so no resolver changed. The consequences are the point: moving a hero takes nothing from them, disbanding takes nothing from anybody, and a hero who joins later inherits no back-catalogue. Fog is the one thing that could not be brushed — a batch holds circles, not per-hero rows — so it records the heroes standing there at submit time in `reveal_batch_heroes` (none = the whole table) and keeps `reveal_batches.party_id` as a **label only**. That replaced `knowledge_pools`, the last veil in the app keyed by *user* rather than by hero. A party also **partitions the roster**: `ListCharacters` and `ListParties` filter to the viewer's own parties, so two groups on different roads stop reading each other's cast list and each other's party names (a hero riding with nobody is the company of everyone else riding with nobody — which is why a campaign with no parties reads exactly as it always did). An ally rides with one party too, via `npcs.party_id` — NULL is the whole table, and the nil UUID puts them back there, absent keeping what is set. The `visible_to_party` columns keep their historic name and still mean *the whole table*.
- **An ally is one of the Folk with a state.** `npcs.traveling` (#228) puts a person beside the party without ever putting them in it — they sit in the roster's own *Traveling with you* section, counted among nobody, because #227's `kind` already keeps them off every hero query. Traveling steps a person out of the **place tree** and nothing else: a traveler is where the party is, not where they are filed, or a veiled home town would hide the man riding beside the cart. Their own veil still rules them completely — setting out opens the party-wide flag *once*, at that moment, and never again, so veiling a traveler works and re-assigning their runner cannot quietly undo it (a switch that cannot say no is not a switch). The one exception is the runner: whoever holds an ally sees them whatever the veil says, because being told to run someone you cannot see is not a state worth having. Their numbers are a separate question and stay behind the stats veil. `npcs.control` (`dm|player|table` + `control_user_id`) says who runs them, and control **carries the numbers** — `runsAlly` lifts the stats veil for the runner alone and opens `PUT /npcs/{id}/hp`, the one door on a person that is not the DM's; a member who does not hold them gets a 404, not a 403. Hit points land where they live: a forged body's on its sheet, a stat-block person's on `npcs.hp_current` (NULL = untouched, maximum read off the block every time). In a fight they are a fourth combatant `kind` — `ally`, seated by `npc_id` — party-side like a `pc`, named to players rather than shown as "Unknown", one fight at a time, and mirrored home on every HP change.
