...
| `webhooks.spec.ts` | a receiver on the docker host: a hook registered on the profile gets a signed delivery for a posted quest and none for a drafted one; a ping; a token-born hook is refused a name past its scopes and never hears one; a dead URL shows a failed attempt with its next try | `internal/webhooks`, the handlers in `webhooks.go`, `WebhooksSettings.tsx` |
