...
const getForgedByKey = `-- name: GetForgedByKey :one
...
type GetForgedByKeyParams struct {
	OwnerUserID uuid.UUID `json:"owner_user_id"`
	ForgeKey    *string   `json:"forge_key"`
}
...
func (q *Queries) GetForgedByKey(ctx context.Context, arg GetForgedByKeyParams) (Character, error) {
	row := q.db.QueryRow(ctx, getForgedByKey, arg.OwnerUserID, arg.ForgeKey)
	var i Character
	err := row.Scan(
		&i.ID,
		&i.CampaignID,
