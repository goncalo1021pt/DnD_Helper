...
FROM reveal_circles c
JOIN reveal_batches b ON b.id = c.batch_id
WHERE b.map_id = sqlc.arg(map_id)
  AND b.campaign_id = sqlc.arg(campaign_id)
  AND (NOT EXISTS (SELECT 1 FROM reveal_batch_heroes h WHERE h.batch_id = b.id)
       OR EXISTS (SELECT 1 FROM reveal_batch_heroes h
                  WHERE h.batch_id = b.id AND h.character_id = ANY(sqlc.arg(character_ids)::uuid[])));
...
FROM reveal_batches b
WHERE b.id = sqlc.arg(batch_id) AND b.campaign_id = sqlc.arg(campaign_id);
